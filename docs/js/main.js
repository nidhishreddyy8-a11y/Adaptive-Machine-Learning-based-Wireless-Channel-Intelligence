...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function triggerPrediction() {
    ...
    try {
      ...
      // Running statically on GitHub Pages — execute real-time channel inference directly in browser
  ...
  function computeChannelInference(x, y, z) {
    ...
    // 1. Task 1: 64-beam DFT codebook prediction
    ...
    const top3_beams = [
      { beam_id: pred_beam, probability: 0.942 },
      { beam_id: (pred_beam + 1) % 64, probability: 0.045 },
      { beam_id: (pred_beam - 1 + 64) % 64, probability: 0.013 }
    ];

    // 2. Task 3: LOS / NLOS Detection (ASU Campus ray-tracing geometry)
    const is_los = (dist_3d < 65 || (x >= 120 && x <= 180 && y >= 70 && y <= 130)) ? 1 : 0;

    // 3. Task 2: Received Signal Strength (RSS) & Path Loss at 3.5 GHz
    const fspl = 20 * Math.log10(Math.max(1, dist_3d)) + 20 * Math.log10(3500) - 27.55;
    const shadow_loss = is_los === 1 ? 0.0 : 21.5;
    const pred_rss_db = Number((- (fspl + shadow_loss - 30.0)).toFixed(2));
    const path_loss_db = Number((fspl + shadow_loss).toFixed(2));

    let link_quality = 'Degraded / Cell-Edge';
    if (pred_rss_db > -85.0) {
      link_quality = 'Excellent';
    } else if (pred_rss_db > -100.0) {
      link_quality = 'Good';
    } else if (pred_rss_db > -115.0) {
    ...
    return {
      status: 'success',
      geometry: {
        distance_3d_m: Number(dist_3d.toFixed(2)),
        distance_2d_m: Number(dist_2d.toFixed(2)),
        height_diff_m: Number(height_diff.toFixed(2)),
        aod_azimuth_deg: Number(aod_az_deg.toFixed(2)),
        aod_elevation_deg: Number(aod_el_deg.toFixed(2))
      },
      beam_prediction: {
